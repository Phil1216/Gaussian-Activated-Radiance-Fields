...
def plot_save_poses(opt,fig,pose,pose_ref=None,path=None,ep=None):
    ...
    setup_3D_plot(ax3,elev=0,azim=-0,lim=edict(x=(-1,1),y=(-1,1),z=(-1,1)))
    ax1.set_title("forward-facing view",pad=0, fontsize=15)
    ax2.set_title("top-down view",pad=0, fontsize=15)
    ax3.set_title("side view",pad=0, fontsize=15)
    #plt.subplots_adjust(wspace=0,hspace=0)
    ...
    for i in range(N):
        if pose_ref is not None:
            ...
            ax3.plot(cam_ref[i,:,0],cam_ref[i,:,1],cam_ref[i,:,2],color=(0.3,0.3,0.3),linewidth=1)
